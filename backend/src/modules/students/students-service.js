...
const { findAllStudents, findStudentDetail, findStudentToSetStatus, addOrUpdateStudent, deleteStudent, deleteStudentProfile } = require("./students-repository");
...
const processDeleteStudent = async (id) => {
    await checkStudentId(id);

    const affectedRowStudentProfile = await deleteStudentProfile(id);
     if (affectedRowStudentProfile <= 0) {
        throw new ApiError(500, "Unable to delete student profile");
    }
